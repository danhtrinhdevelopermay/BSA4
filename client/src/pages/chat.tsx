  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validFiles.length !== files.length) {
      toast({
        ...
        description: "Chỉ chấp nhận tệp ảnh và video.",
        variant: "destructive",
      });
    }
    ...
    if (selectedFiles.length > 0) {
      try {
        for (const file of selectedFiles) {
          ...
          // For videos, we'll use the data URL as both url and thumbnailUrl for now
          // In production, you'd want to generate proper thumbnails
          mediaUrls.push({
            type,
            url: dataUrl,
            thumbnailUrl: type === 'video' ? dataUrl : undefined
          });
        }
      } catch (error) {
        toast({
          title: "Lỗi",
          description: "Không thể xử lý tệp. Vui lòng thử lại.",
          variant: "destructive",
        });
        return;
      }
